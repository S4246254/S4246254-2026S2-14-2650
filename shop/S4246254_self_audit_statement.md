...
nine contact and delivery fields, for WCAG 1.3.5 Identify Input Purpose.

The four payment fields are a deliberate exception, carrying `autocomplete="off"` rather
than `cc-name`, `cc-number`, `cc-exp` and `cc-csc`. Those tokens prompt a browser to offer
the reader's real stored card, and nobody should be one click from autofilling their own
card into a checkout that takes no payment. I judged that more important than the
conformance point, and scoped the loss to those four fields.
...
and would need server-side handling to identify which was pressed. Removing the card
tokens reduces rather than eliminates autofill, since browsers infer card fields from
labels.

*Prose word count: 261, measured excluding headings and the metadata lines above.*
